App = {
  ...
  handleInvioDoc: async function (event) {
    ...
    web3.eth.getAccounts(function (error, accounts) {
      ...
      App.contracts.DocCert.deployed()
        .then(function (instance) {
          docCertInstance = instance;

          return docCertInstance.addFile(hash.toString(), cid.path.toString(), {
            from: account,
          });
        })
        ...
        .catch(function (err) {
          console.log(err);
          alert(
            "Non è stato possibile aggiungere il documento all BlockChain."
          );
        });
    });
  },
      ...
      App.contracts.DocCert.deployed()
        ...
        .then(function (result) {
          ...
          document.getElementById("addressText").innerHTML =
            "Address : " + result[0].toString().replaceAll(",", "\n");
          ...
          document.getElementById("TimeStampText").innerHTML =
            "TimeStamp : " + formattedTime;
          document.getElementById("BlockNumberText").innerHTML =
            "BlockNumber : " + result[2].c[0];
        })
        .catch(function (err) {
    ...
    if (/^0x[0-9a-f]{40}/.exec(newAdress.toLowerCase())) {
      web3.eth.getAccounts(function (error, accounts) {
        if (error) {
          console.log(error);
          alert(
            "Non è stato possibile recuperare le informazioni sull'account"
          );
        }
        ...
        App.contracts.DocCert.deployed()
          ...
          .catch(function (err) {
            console.log(err.message);
            alert(
              "Non è stato possibile modificare le informazioni sul documento"
            );
          });
      });
    } else {
